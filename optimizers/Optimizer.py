...
if __name__ == '__main__':
    ...
    #                                        params=sys_params)
    trading_algo = taf.create_trading_algo(params)

    # Create and run optimizer
    optimizer = of.create_optimizer(params)
    # optimizer = of.create_optimizer(opt_name=opt_name, opt_params=opt_params, sys_params=sys_params)
    print('Optimizing parameter set for dates %s to %s.' % (params.start_date, params.end_date))
    # print 'Optimizing parameter set for dates %s to %s.' % (start_date, end_date)
    start_time = time.time()
    results = optimizer.run(trading_algo=trading_algo, start_date=params.start_date.date(), end_date=params.end_date.date())
    end_time = time.time()
