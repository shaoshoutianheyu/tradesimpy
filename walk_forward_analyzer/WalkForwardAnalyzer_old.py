...
class WalkForwardAnalyzer(object):
    ...
    def run(self):
        ...
        # Create trading algorithm, optimizer, and backtester
        trading_algo = taf.create_trading_algo(params=self.params)
        optimizer = of.create_optimizer(params=self.params)
        backtester = b.Backtester(capital_base=capital_base, commission=self.params.commission,
        ...
        # Perform walk forward analysis with trading algorithm
        for per in self.sample_periods:
            in_start = per['in'][0]
            in_end = per['in'][1]
            out_start = per['out'][0]
            ...
            start_time = time.time()
            # period_results, daily_results = backtester.run(capital_base=capital_base, trading_algo=trading_algo, data=data)
            daily_results = backtester.run(capital_base=capital_base, trading_algo=trading_algo, data=data)
            end_time = time.time()
            print('Finished out-of-sample simulation %f seconds.\n' % (end_time - start_time))

            # Keep track of the portfolio's value over time
            capital_base = daily_results['Cash'][0] + daily_results['Invested'][0]
            ...
            portfolio_series = pd.concat([portfolio_series, daily_results])
...
if __name__ == '__main__':
    ...
    walk_forward_analyzer = WalkForwardAnalyzer(params)
    portfolio_stats, portfolio_series = walk_forward_analyzer.run()
