...
class TradeDecision(object):
	def __init__(self, ticker, open_or_close, long_or_short=None, share_count=None, position_percent=None):
		...
		if(share_count):
			...
			raise AttributeError("You must provide either a share count or position percent in TradeDecision")
		...
		if(self.open_or_close == 'open'):
			if(long_or_short != 'long' and long_or_short != 'short'):
				raise AttributeError("The TradeDecision parameter long_or_short must be set to either 'long' or 'short': %s" % long_or_short)
			else:
